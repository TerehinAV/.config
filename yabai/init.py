...
class Paths:
    ...
    xonsh: str = "/opt/homebrew/bin/xonsh"
...
class Yabai:
    ...
    def query_windows(self) -> list[dict]:
        result = subprocess.run(
            [self._path, "-m", "query", "--windows"], capture_output=True, text=True
        )
    ...
    def find_window_by_id(self, wid: int) -> dict | None:
    ...
    def find_window(self, predicate: Callable[[dict], bool]) -> dict | None:
...
class WeztermRule(WindowRule):
    ...
    def launch(self) -> None:
        ...
        subprocess.Popen(
            [self.paths.wezterm, "start", "--", self.paths.xonsh, "-c", full_cmd]
        )
...
class BrowserRule(WindowRule):
    ...
    def find_existing(self) -> dict | None:
        return self.yabai.find_window(
            lambda w: (
                w.get("app") == self.app and self.title_pattern in w.get("title", "")
            )
        )
